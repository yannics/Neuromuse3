...
\begin{tabular}{r*1{c>{\ttfamily}l}cll}
  ...
  &  \faCog & corpus & {\footnotesize \textit{string}$|$\textit{function}} &  $\rightarrow$ \glspl{set-corpus}  \\
        &  \faCog & remanence & \itshape boolean$|$integer & \tablefootnote{Concerning the remanence, there is four specific cases. When unset (1) there is still a default value set to one as the last event sent. When set as boolean and when the \texttt{net} is set, (2) the remanence is the value of \texttt{*remanence*}  (global value from N3D). When remanence is set as an integer, then (3) the remanence is this value. Finally, when the slot corpus is set, the remanence is set on the keyword \texttt{:corpus} (this might conflict with net, so for now it is recommended to dedicate sequencing for either a net or a corpus). The corpus itself is a data file path (stored in "$\sim$/Documents/neuromuse3-backup-networks/data/" by default).}  \\
...
%        &   & outset & \itshape boolean &  \\
 ...
 % &   & counter & \itshape integer &   \\
    &  \begin{minipage}{.025\textwidth}\includegraphics[width=\linewidth]{1123}\end{minipage} & osc-in &  \textit{integer} &  {\footnotesize port}  \\ 
