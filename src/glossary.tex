...
\newglossaryentry{copy}{
	...
	plural={\texttt{copy}},
	description={
	\texttt{self}\myuline{[som$|$area$|$sequencing]} 
	\\ \textsl{{\&}key} \texttt{newname}\myuline{[symbol]} \texttt{with}\myuline{[list]}   \texttt{reset}\myuline{[bool]}
	\\ $\rightarrow$ \textit{nil}  
	\vspace{2mm}  \\ The optional keywords \texttt{:with} and \texttt{:reset} concern only \textit{self} as an AREA. The argument of \texttt{:with} is a new  \texttt{soms-list} as a list of SOM new names -- which should be the same length of the \texttt{soms-list} of \textit{self}. ..}\bigskip
	}
...
\newglossaryentry{create-mlt}{
	...
	\\ \textsl{{\&}key} \texttt{carte}\myuline{[function]} \texttt{topology}\myuline{[int]} \texttt{field}\myuline{[num$|$list]}  \texttt{cover-value}\myuline{[int]}\\ \texttt{n-fanaux}\myuline{[int]} 
	...
	\\ \texttt{:topology} -- Number of dimensions.
	\\ \texttt{:field} -- List of maximum value by dimension or maximum value for all dimensions (by default 10 with \glspl{rnd-map} and $\lfloor \sqrt[d]{N} \rfloor$ with \glspl{quadrare}).
	\\ \texttt{:cover-value} -- length of the MCT.
...
\newglossaryentry{create-sequencing}{
	...
	description={
	\textsl{{\&}key} \texttt{dub}\myuline{[symbol]} \texttt{description}\myuline{[string]} \texttt{net}\myuline{[som$|$mlt$|$area]}   \texttt{remanence}\myuline{[bool]}\\ \texttt{buffer-size}\myuline{[int]} \texttt{buffer-thres}\myuline{[int]}   \texttt{pattern}\myuline{[list]}  \texttt{odds}\myuline{[function]}  \texttt{sync}\myuline{[bool]} \texttt{meter}\myuline{[num]} \texttt{pulse}\myuline{[num]} \texttt{tag}\myuline{[symbol]}  \texttt{osc-in}\myuline{[list$|$int]} \texttt{osc-out}\myuline{[list$|$int]}
...
\newglossaryentry{set-corpus}{
	name={\texttt{SET-CORPUS}},
        plural={\texttt{set-corpus}},
	description={
	\texttt{self}\myuline{[sequencing]} \texttt{corpus}\myuline{[string$|$pathname$|$symbol$|$list$|$\{lambda*\}]}
	\\ $\rightarrow$  \textit{nil}  
	\vspace{1mm} \\ ..}\bigskip
	}
...
\newglossaryentry{set-pattern}{
	name={\texttt{SET-PATTERN}},
        plural={\texttt{set-pattern}},
	description={
	\texttt{self}\myuline{[sequencing]} \texttt{lst}\myuline{[list]}
	\\ $\rightarrow$  \textit{list}  
	\vspace{1mm} \\ ..}\bigskip
	}
...
\newglossaryentry{set-subroutine}{
	...
	description={
	\texttt{self}\myuline{[sequencing]} \texttt{subroutine}\myuline{[sequencing]} 
	\\ \textsl{{\&}key} \texttt{sync}\myuline{[bool]} \texttt{anacrusis}\myuline{[int]}  
	\\ $\rightarrow$  \textit{table}  
	\vspace{1mm} \\ ..}\bigskip
	}

\newglossaryentry{set-sync}{
	name={\texttt{SET-SYNC}},
        plural={\texttt{set-sync}},
	description={
	\texttt{follower}\myuline{[sequencing]} \texttt{leader}\myuline{[sequencing]}
	\\ \textsl{{\&}key} \texttt{anacrusis}\myuline{[int]}  \texttt{save}\myuline{[bool]} 
	\\ $\rightarrow$  \textit{table}  
	\vspace{1mm} \\ ..}\bigskip
	}
