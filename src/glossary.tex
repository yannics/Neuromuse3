...
\newglossaryentry{mapping}{
	...
	\\ \texttt{:init-rad} -- Initial \texttt{radius} (mean value of the \texttt{field} of \textit{self} divided by 2).
	\\ \texttt{:end-rad} -- Final \texttt{radius} (0.1 by default).
	\\ \texttt{:dfun} -- Decreasing function (\glspl{exp-decay} by default).
	\\ \texttt{:dtype} -- Normalisation type applied to the \textit{dataset} (\texttt{:flat} by default). ..}\bigskip
	}
...
\newglossaryentry{scaling}{
	...
	\\ \texttt{:mlt} -- Allows to set the slot \texttt{data-scale} in order to retrieve initial data values or to keep track of the data range according to the MLT as this value. In that case, only the default values of keys are taken into account.
	\\ \texttt{:data-type} -- Specify the normalisation type such as by dimension \texttt{:dim} (set as a list of intervals), only 0/1 values \texttt{:bin} (set as 0), by event \texttt{:std} (set as 1) or by default \texttt{:flat} (set as an interval). ..}\bigskip
	}
