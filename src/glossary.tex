...
\newglossaryentry{mapping}{
	...
	\\ \textsl{{\&}key} \texttt{init-lr}\myuline{[num]} \texttt{end-lr}\myuline{[num]} \texttt{init-rad}\myuline{[num]} \texttt{end-rad}\myuline{[num]} \\ \texttt{dfun}\myuline{[function]} \texttt{dtype}\myuline{[keyword]}
	\\ $\rightarrow$ \textit{No value} 
	\vspace{2mm} 
	...
	\\ \texttt{:dfun} -- Decreasing function (\glspl{exp-decay} by default).
	\\ \texttt{:dtype} -- Normalisation type applied to the \textit{dataset} (see description of the key \texttt{:data-type} within the function \glspl{scaling}). ..}\bigskip
	}
...
\newglossaryentry{scaling}{
	...
	\\ \textsl{{\&}key} \texttt{minin}\myuline{[num]} \texttt{maxin}\myuline{[num]} \texttt{minout}\myuline{[num]} \texttt{maxout}\myuline{[num]} \texttt{mlt}\myuline{[mlt]} \\ \texttt{data-type}\myuline{[keyword]}
	\\ $\rightarrow$ \textit{num}$|$\textit{list}  
	\vspace{2mm} \\ Normalize \textit{data} value(s) occuring between \texttt{:minin} and \texttt{:minout} -- as minimal and maximal values of the dataset by default -- within the interval \texttt{:minout} and \texttt{:maxout}, respectively $0$ and $1$ by default.  
	\\ \texttt{:mlt} -- Allows to retrieve initial data values or to keep track of the data range according to the MLT as this value. In that case, only the default values of keys are taken into account except the key \textit{data-type} if it is specified. 
	\\ \texttt{:data-type} -- Specify the normalisation type (store in the slot \texttt{data-scale} of the key \textit{mlt}) such as by dimension \texttt{:dim} (set as a list of intervals), only 0/1 values \texttt{:bin} (set as 0), by event \texttt{:std} (set as 1) or according to all values of \textit{data} \texttt{:flat} (set as an interval). If this key is not specified, the normalisation is done according to \texttt{data-scale}. If the latter is \textit{nil}, then the default key value will be set as \texttt{:flat}.  ..}\bigskip
	}
