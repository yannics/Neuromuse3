...
\begin{itemize}
\item[] \myuline{the range} which is estimated by default between the minimal value and the maximal value of the dataset (any other values outside the range will be clipped);
\item[] \myuline{the curve} allows to set the scale of the input data such as if this value equal to 0 (linear) then: 
...
\item[] \myuline{the type} of the data input as:
\begin{itemize}
