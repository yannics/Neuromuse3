...
Let a corpus of partial sequences as MDS \citep{yi} -- Chapter 3 \textsl{Music Data Score} -- formatted as follow 
\begin{lstlisting}[language=N3]
(((seq1-dur_1 seq1-dur_2 ... seq1-dur_n) 
  (seq1-midi_1 seq1-midi_2 ... seq1-midi_n)) 
 ((seq2-dur_1 seq2-dur_2 ... seq2-dur_n) 
  (seq2-midi_1 seq2-midi_2 ... seq2-midi_n)) 
 ... 
 ((seqN-dur_1 seqN-dur_2 ... seqN-dur_n) 
  (seqN-midi_1 seqN-midi_2 ... seqN-midi_n)))
  
\end{lstlisting}
 be the experience in the area network defined by cliques of order 3 --  i.e. one node for the duration of 8 units, one node for the degree in modulo 12 and one node concerning the interval in semi-tone with the next note between $-11$ and $+11$ corresponding to a total number of 23.
\begin{lstlisting}[language=N3]
(defparameter +corpus+ (loop for i in MDS collect 
   (mat-trans (list 
      (car i) 
      (loop for x in (cadr i) collect (mod x 12)) 
      (complist (x->dx (cadr i)) (car i) nil)))))
\end{lstlisting}
From this point, each sequence of the \texttt{+corpus+} looks like 
\begin{lstlisting}[language=N3]
   ...
   do     
	...
	for i from 0 to (1- (length (car seq))) do
	  (loop
	     ...
	     for d in seq do
	       (setf (input (id s)) (scaling (nth i d) :mlt (id s))))
	  (learn CORPUS))
